function confirmlink455(linkname) {
    if (confirm("Are you sure you want to jump to " + linkname + "?")) location = linkname;
}
...
function getBreaks661() {
    var userInput;
    var breaksString = "";
    do userInput = parseInt(prompt("Enter a number from 1 to 5:"));
    while (userInput < 1 || userInput > 5);
    for(var i = 0; i < userInput; i++)breaksString += "<br>";
...
function getCards503() {
    var userInput;
    var cardsString = "";
    do userInput = parseInt(prompt("Enter a number from 2 to 7:"));
    while (userInput < 2 || userInput > 7);
    for(var i = 0; i < userInput; i++)cardsString += "Card #" + i + " ";
    var cards503Span = document.getElementById("cards503");
    cards503Span.innerHTML = cardsString;
}

//# sourceMappingURL=index.342d1e9b.js.map
